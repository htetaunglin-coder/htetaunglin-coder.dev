...
- **One post embeds a tool.** `banner-prompt-spec` renders `<BannerPrompt />` (`src/features/blog/components/banner-prompt/`) — Fumadocs `Tabs` with a **Build it** form and a **Raw prompt** code block. The spec text lives in `src/features/blog/lib/banner-prompt/spec.ts`, split into four regions; `compile.ts` reuses the generation rules and self-check without the interview step, so the two tabs never drift. Do **not** paste the spec back into the MDX.
  ...
  - **The widget is a fixed box; the steps inside it are not.** Measured, the steps run 131–399px on desktop and 192–825px on mobile, so no single panel height serves them — pinning one left 253px blank under the shortest step while the tallest still scrolled. The box scrolls instead, the open step takes its natural height, and the fade ramp under it absorbs the slack. Two traps, both of which read as tidying: the step panel must not shrink, or flex squashes its box while the step keeps its real height and the cards spill over the ramp; and the ramp needs a minimum height, or a step taller than the box leaves it nothing to grow into and the run appears to stop dead.
  - The panel renders `compileBriefSummary()`, not the full prompt: the compiled output is ~21k characters of fixed house rules the reader never chose. Copy and download still hand over `compileBannerPrompt()`.
