...
- Pages:
  ...
  - Workshop pages: `src/app/(app)/(main)/skills/page.tsx`, `src/app/(app)/(narrow)/skills/[slug]/page.tsx` and `src/app/(app)/(main)/design-system/page.tsx`. Three routes, one feature (`src/features/workshop`), because the header groups them as one thing. `/skills` lists the `SKILL.md` folders of the public `htetaunglin-coder/skills` repo as Blog-style rows (`skills-index-view.tsx`) — name, description, `/ Published · license`, `View skill →`. `/skills/[slug]` is the detail (`skill-detail.tsx`). It sits in the `(narrow)` route group, not `(main)`, because `(main)/layout.tsx` renders a 4xl footer under a 3xl column; `(narrow)/layout.tsx` sets `--content-max-width` once and the page column and the footer both read it. The page shows a mono eyebrow, the gradient heading with the repo link beside it, the description, then the per-skill `--skill` install line — installing is the page's job, so it sits above the fold. Below it the SKILL.md body renders like a blog post (`SkillContent` in `skill-detail.tsx`): Fumadocs' runtime `Markdown` with `remarkHeading` and `rehypeCode` and the default MDX components, inside a `.blog` wrapper so the article overrides in `globals.css` apply. Relative links resolve against the skill's folder on GitHub (`baseUrl`). It is not a blog collection entry on purpose: SKILL.md is plain Markdown fetched at runtime, a bare `<…>` in prose breaks MDX, and the blog schema requires `date`, `image` and `series`. The command appears once and does not follow the reader: a sticky install bar was tried and cut — it floated over the content, and one command at the top is enough. Status and license are not shown on the detail: chips gave those facts more weight than they carry. The install command is deliberately absent from the index: `npx skills add owner/repo` installs every skill in the repo, half-done ones included. The tree and raw files come from GitHub with `next: { revalidate: 86_400 }`, so both routes are SSG (`generateStaticParams`) with daily ISR; `dynamicParams = true` lets a folder pushed after the build render on first visit. Frontmatter is parsed for name/description/license (`src/features/workshop/api/github-skills.ts`, same pattern as `home/api/contribution.ts`). A fetch failure fails open to an empty list with a logged error; the static `SKILLS` entries in `src/features/workshop/data.ts` always render below as **Working on** and have no detail page. The status comes from the source, not the data: GitHub rows are published, `SKILLS` rows are in progress. `sitemap.ts` lists the detail URLs; that fetch's daily `revalidate` overrides the route's `revalidate = false`, so the sitemap now refreshes daily too. Backtick spans in SKILL.md text go through `skill-text.tsx`, which overrides `BasicMarkdown`'s handwritten code style with mono. Both index pages open with `workshop-hero.tsx`: a 440px band with a full-bleed photo from `public/` (`hero-skills.jpg`, `hero-design-system.jpg`, loaded with `next/image`, not Cloudinary) behind a mono eyebrow (`Workshop / 01`), the title and the description, in a `max-w-5xl` column — wider than the site's 4xl so the workshop reads as its own document. The photos carry their own black ground, so a radial mask fades them out before the box edge; light mode gets the index heroes' top-to-bottom overlay, dark mode a wash from the left and the bottom. `/design-system` shows only its message under the hero. Neither is linked from the main nav — both sit in the header's `More` panel under **Workshop**, flagged `soon` in `src/constants/navigation.ts`. `SKILLS_PAGE` and `DESIGN_SYSTEM_PAGE` there are the one source for each page's route, title and hero photo — the nav tile shows the same photo as the page — and `skillPath(slug)` builds the detail URL. Drop that flag when the page stands on its own
    - The header's **More** panel tiles use the same two hero photos as backdrops (`image` on each `WorkshopItem` in `src/constants/navigation.ts`, with a per-photo `position` so the subject stays in the crop), under a top-down `bg-default-alt` gradient that keeps the tile copy readable
    - The strokes are `#8f8bb4`, copied from `decorations/profile.tsx` rather than a theme token, so the tiles match the hero portrait exactly in both themes. Both would move together if that colour is ever tokenised
