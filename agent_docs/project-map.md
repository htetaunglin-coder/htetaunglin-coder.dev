...
- Pages:
  ...
  - Workshop pages: `src/app/(app)/(main)/skills/page.tsx` and `src/app/(app)/(main)/design-system/page.tsx`. Two routes, one feature (`src/features/workshop`), because the header groups them as one thing and they share a card, a page shell and an entry shape. Content is static data in `src/features/workshop/data.ts`; `status` drives the card badge, so an entry going live is a data edit, not a component edit. Neither is linked from the main nav — both sit in the header's `More` panel under **Workshop**, flagged `soon` in `src/constants/navigation.ts`. Drop that flag when the page stands on its own
    - Tile backdrops: `src/components/decorations/workshop-illustrations.tsx`, drawing static paths from `workshop-sketch-art.ts`. The isometric geometry is run through rough.js's line algorithm — each edge becomes two bowed beziers with jittered endpoints, which is what makes joins overshoot like a pen. Two knobs, easy to confuse: endpoint scatter (roughness x offset) makes corners fly apart, bow (bowing x offset) makes lines waver. Shipped at low roughness and high bowing, so lines squiggle while corners hold. The PRNG is seeded, so regenerating is deterministic
