...
- Shells:
  ...
  - `src/app/(app)/(narrow)/layout.tsx` — a 3xl column; sets `--content-max-width` once so the page and the footer share one width
- Pages:
  ...
  - Workshop pages: `src/app/(app)/(main)/skills/page.tsx`, `src/app/(app)/(narrow)/skills/[slug]/page.tsx` and `src/app/(app)/(main)/design-system/page.tsx`. Three routes, one feature (`src/features/workshop`), because the header groups them as one thing. `/skills` lists the `SKILL.md` folders of the public `htetaunglin-coder/skills` repo as Blog-style rows (`skills-index-view.tsx`) — name, description, `/ Published · license`, `View skill →`. `/skills/[slug]` is the detail (`skill-detail.tsx`). It sits in the `(narrow)` route group, not `(main)`, because `(main)/layout.tsx` renders a 4xl footer under a 3xl column; `(narrow)/layout.tsx` sets `--content-max-width` once and the page column and the footer both read it. The page shows a mono eyebrow, the gradient heading, the description, then the per-skill `--skill` install line with the GitHub link in its header — installing is the page's job, so it sits above the fold. Below it the SKILL.md body renders like a blog post (`SkillContent` in `skill-detail.tsx`): Fumadocs' runtime `Markdown` with `remarkHeading` and `rehypeCode` and the default MDX components, inside a `.blog` wrapper so the article overrides in `globals.css` apply. Relative links and images resolve the way GitHub resolves them (`resolveSkillUrl` in `github-skills.ts`): against the skill's folder, or against the repo root for `/…`. Links open the file on GitHub. Images load the raw file through a plain `<img>`: a SKILL.md image has no known size and is not on Cloudinary, so neither `cloudinary-image.tsx` nor `next/image` fits. It is not a blog collection entry on purpose: SKILL.md is plain Markdown fetched at runtime, a bare `<…>` in prose breaks MDX, and the blog schema requires `date`, `image` and `series`. The command appears once and does not follow the reader: a sticky install bar was tried and cut — it floated over the content, and one command at the top is enough. Status and license are not shown on the detail: chips gave those facts more weight than they carry. The install command is deliberately absent from the index: `npx skills add owner/repo` installs every skill in the repo, half-done ones included. The tree and raw files come from GitHub with `next: { revalidate: 86_400 }`, so both routes are SSG (`generateStaticParams`) with daily ISR; `dynamicParams = true` lets a folder pushed after the build render on first visit. Frontmatter is parsed for name/description/license (`src/features/workshop/api/github-skills.ts`, same pattern as `home/api/contribution.ts`). It is not a YAML parser: indented lines join onto the key above, so a `>-` description reads as one line. A GitHub failure is logged. During `next build` and in dev it fails open to an empty list, so an outage never blocks a deploy. On the production server it throws, so ISR keeps serving the last good page — an empty list would be cached for a day, and every skill page would 404. The optional `GITHUB_TOKEN` lifts the tree API's limit of 60 requests an hour per IP. The static `SKILLS` entries in `src/features/workshop/data.ts` always render below as **Coming soon** and have no detail page. The status comes from the source, not the data: GitHub rows are published, `SKILLS` rows are in progress. `sitemap.ts` lists the detail URLs and revalidates daily to match that fetch. Backtick spans in SKILL.md text go through `skill-text.tsx`, which overrides `BasicMarkdown`'s handwritten code style with mono. Both index pages open with `workshop-page.tsx` — `workshop-hero.tsx` plus the divided column under it. The hero is a 440px band with a full-bleed Cloudinary photo (`hero-skills_pseme1.jpg`, `hero-design-system_qcmobx.jpg`, through `cloudinary-image.tsx`) behind a mono eyebrow (`Workshop / 01`), the title and the description, in a `max-w-5xl` column — wider than the site's 4xl so the workshop reads as its own document. The photos carry their own black ground, so a radial mask fades them out before the box edge; light mode gets the index heroes' top-to-bottom overlay, dark mode a wash from the left and the bottom. Mobile hides the photo, so its `sizes` gives mobile a 1px slot and the preload fetches the smallest file there. `/design-system` shows only its message under the hero (`design-system-view.tsx`). Neither is linked from the main nav — both sit in the header's `More` panel under **Workshop**, flagged `soon` in `src/constants/navigation.ts` (no UI reads the flag at the moment). `SKILLS_PAGE` and `DESIGN_SYSTEM_PAGE` there are the one source for each page's route, title and hero photo — the nav tile shows the same photo as the page — and `skillPath(slug)` builds the detail URL. Drop that flag when the page stands on its own
    - The header's **More** panel tiles use the same two hero photos as backdrops (`image` on each `WorkshopItem` in `src/constants/navigation.ts`, with a per-photo `position` so the subject stays in the crop), on a `bg-media` ground under a top-down `bg-media` gradient, with `fg-media` copy, so the tile reads the same in both themes
...
- `GITHUB_TOKEN` — optional; authenticates the skills repo tree request (`src/features/workshop/api/github-skills.ts`)
