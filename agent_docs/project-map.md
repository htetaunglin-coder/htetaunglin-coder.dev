...
- Entry to a cluster is gated on `!draft`, at **both** ends. A draft is unpublished writing — held out of the sitemap and the search index, and `noindex` on Burmese routes — and an `hreflang` advertises a URL to a crawler. Narrower than `LanguageSwitch`'s test on purpose: a draft translation is still worth linking to for a reader looking straight at it. **Note the asymmetry:** `blog/[slug]` sets no `robots`, so an English draft still prerenders indexable; only the sitemap and search index exclude it. Marking an English post `draft: true` while its Burmese translation is live would silently drop the pair's cluster. No English drafts exist today; noindexing them is a separate fix.
