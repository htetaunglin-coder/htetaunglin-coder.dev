...
description: "This blog explains how React turns the code we write into the UI users see. We'll explore the full rendering process, including Reconciliation, Render Phase, and Commit Phase."
image:
  {
...
<Accordions type="single">
  <Accordion id="1-component's-state-changes" title="1. Component's State Changes">
...
Let's say, we have a Counter Component with the button named `Add`.

When a user clicks the button, `handleClick` calls `setCount(1)` and updates the `count` state.

Once the state changes, React needs to show the updated value to the screen, so it starts the render process again. (re-render)

  </Accordion>

<Accordion
  id="2-parent-component-re-renders"
  title="2. Parent Component Re-renders">
  in progress... :)
</Accordion>

  <Accordion id="3-context-value-changes" title="3. Context Value Changes">
    in progress... :)
  </Accordion>
